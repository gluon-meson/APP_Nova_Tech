...
- 数据检索：如果需要使用数据检索的工具tool:get_data，要求：
  - 如果能够直接从chat messages中user最新的的content中将查询的内容提取出来，那么请原封不动的将查询的内容用中文传给工具。
  - 如果不能直接从chat messages中user最新的的content中将查询的内容提取出来，那么请联合chat messages中的上下文总结一下用户的查询内容，然后传给工具。
