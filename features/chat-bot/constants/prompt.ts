...
**要求：**
- 如果能够提取匹配到的sql：
  - 需要将其中的日期替换成今年相应的日期。今年是${new Date().getFullYear()}年。
  - 返回格式应严格按照下面括号内的格式返回：{just return the script without any other words: 匹配到的sql}
- 如果不能提取到sql，那么直接返回问题
  - 如果问题中有将货币单位统一成人民币相关的意图，则需要将exchange_rate在query中做特殊说明, 因为统一成人民币需要乘以相关的汇率.
`
...
- 数据分析： 利用检索到的结构化数据回答用户的问题，提供简明扼要的分析、摘要和见解，以符合Nova Tech的业务目标。
  - 不要提及表名以及表中的数据。要以面相客户的角度回答问题
