...
import { K_LINE_DATA, SALES_ORDER_ITEM } from '@/features/chat-bot/types'
...
export const deduplicateItemsByDate = (
  data: KB_QUERY_RESP<SALES_ORDER_ITEM>,
): KB_QUERY_RESP<SALES_ORDER_ITEM> => {
  const uniqueItemsByDate = data.items.reduce(
    (acc, item) => {
      acc[item.approval_date] = item
      return acc
    },
    {} as { [date: string]: SALES_ORDER_ITEM },
  )
...
// }
//
// export const getKeyInfoFromData = (data: KB_QUERY_RESP<SALES_ORDER_ITEM>) => {
